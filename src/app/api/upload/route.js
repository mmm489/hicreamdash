...
export async function POST(request) {
  try {
    ...
    if (!date || !products || !Array.isArray(products)) {
      return NextResponse.json({ success: false, error: 'Missing date or products array' }, { status: 400 })
    }
    ...
    return NextResponse.json({ success: true, date, products: products.length })
  } catch (error) {
